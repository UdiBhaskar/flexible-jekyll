...
\\( \sigma ^2 \\) = \\( \frac{1}{n} \sum_{i=0}^n ({F_1'^T.x_i})^2\\). so our opimization problem is have to find F1' that maximizes variance such that \\( F_1' \\)  is unit vector. so optmization is Max \\( \frac{1}{n} \sum_{i=0}^n ({F_1'^T.x_i})^2\\) S.T \\(F_1'^T.F_1\\) = 1.
