...
\\[ \xi_i = \begin{cases}
1 \; + abs(W^Tx_i+b/||W||) \quad \text{ for incorrectly classified points } \newline 
0 \quad \text{ for correctly classified points } \newline 
\end{cases}\\]
for incorrectly classified points the absolute distance can be written as \\(-y_i(W^Tx_i+b)/||W||\\),  because \\(W^Tx_i+b/||W||\\) gives signed distance and and \\(y_i\\) is +1 for positive class and -1 for negative class. For positive class points in negative region \\(W^Tx_i+b/||W||\\) is negatative,\\(y_i\\) is positive so  \\(-y_i(W^Tx_i+b)/||W||\\) is positive. for negative class points in positive region \\(W^Tx_i+b/||W||\\) is positive,\\(y_i\\) is negative so  \\(-y_i(W^Tx_i+b)/||W||\\) is positive.
\\[ \xi_i = \begin{cases}
