...
Runtime complexity is \\(\;O(kd)\;\\) here k is no of support vectors, d is dimensions beause \\(\;f(x_q)=\sum_{i=1}^n\alpha_iy_ix_ix_q+b\;\\) and \\(\;\alpha_i = 0\\) for non suport vectors
